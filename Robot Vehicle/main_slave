...
{
	char buff[80];
	char recieved_line [80];
    int cnt=0;
    int recieve_x; //1 if recieving x voltage, 0 if recieving y
    float calc;
     char buf[32];
    int npwm;
   ...
   // 	printf("PWM2 (60 to 255): ");
    ...
    //	egets_echo(buf, 31); // wait here until data is received
 	//	printf("\r\n");
		...
		{
			printf("test\n");
			egets2(recieved_line, sizeof(recieved_line)); //read line from serial
			printf("%s", recieved_line);
			if(recieved_line[0] == 'M') //controller is requesting metal detector status
			{ 
				waitms(5); //This delay between reception and transmission may be needed for reliable communication.
				
				//needs to be replaced xbut here for testing
				sprintf(buff, "12345\r\n"); // printf() and friends take a lot of resources and time!  Replace if possible.
				eputs2(buff);
				printf("sent");
			}
			
			else{ //controller is sending input voltages
				calc = atof(recieved_line);
				if(calc>=1.65)
				{
					pwm2=1;
